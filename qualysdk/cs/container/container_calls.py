...
def list_containers(auth: TokenAuth, page_count: Union[int, 'all'] = 'all', **kwargs) -> BaseList[Container]:
    """
    ...
    while True:
        ...
        if response.status_code != 200 and response.text:
            ...
            raise QualysAPIError(f"An error occurred with status code {response.status_code}. Qualys did not return any additional information.")
        ...
        if not headers.get('Link'):
            ...
            pagination_query = parse_qs(headers['Link'].split(';')[0].strip('<>'))['paginationQuery'][0]
