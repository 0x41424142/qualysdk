...
def get_patches_backend(auth: TokenAuth, platform: Literal["windows", "linux"] = "Windows", _ResponsesList: BaseList = BaseList(), **kwargs) -> None:
    """
    ...
    none_keys = [key for key in payload if payload[key] is None] + [key for key in headers if headers[key] is None] + [key for key in params if params[key] is None]
...
def get_patches(auth: TokenAuth, platform: Literal['all', 'windows', 'linux'] = 'all', page_count: Union[int, 'all'] = 'all', **kwargs) -> BaseList:
    """
